...
async function searchProducts(req, res) {
  try {
    ...
    const { keyword, page = 1, limit = 10 } = req.body; // Default page is 1, and limit is 10
    ...
    // Calculate how many items to skip for the current page
    const skip = (page - 1) * limit;
    ...
    // Search in Products collection with pagination
    ...
    // Search in OTC collection with pagination
    ...
    // Total counts for Products and OTC collections (for pagination metadata)
    const productsCount = await productsCollection.countDocuments(searchQuery);
    const otcCount = await otcCollection.countDocuments(searchQuery);
    const totalResults = productsCount + otcCount;
